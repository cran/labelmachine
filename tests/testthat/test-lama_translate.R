...
test_that("'lama_translate_' works with 'to_factor = FALSE'", {
  ...
  df_new <- lama_translate_(
    ...
    col_new = c("var_r_new", "var_r", "var_t_new", "var_t"),
    keep_order = c(TRUE, FALSE, TRUE, FALSE),
    to_factor = c(TRUE, FALSE, FALSE, FALSE)
  )
  expect_s3_class(df_new, "data.frame")
  expect_column_names(
    df_new, 
  ...
  expect_factor_levels(df_new$var_r_new, c("F", "X", "D", "A", "B"))
  expect_identical(as.character(df_new$var_r_new), df_new$var_r_labeled)
  expect_identical(df_new$var_r, df_new$var_r_labeled)
  expect_identical(df_new$var_t_new, df_new$var_t_labeled)
  expect_identical(df_new$var_t, df_new$var_t_labeled)
  df_new_2 <- lama_translate_(
    df,
    dict,
    translation = c("r._", "s"),
  ...
  expect_identical(df_new_2$var_r, df_new_2$var_r_labeled)
  expect_identical(df_new_2$var_s, df_new_2$var_s_labeled)
  df_new_3 <- lama_translate_(
    df,
    dict,
    translation = "var_t",
